...
__all__ = ["register_adv_est", "get_adv_estimator_fn", "AdvantageEstimator", "compute_reward_history_stats"]
...
def compute_reward_history_stats(reward_history: list[float] | np.ndarray) -> dict[str, float]:
    """Compute mean and variance for historical per-batch reward signals."""
    reward_arr = np.asarray(reward_history, dtype=np.float32)
    if reward_arr.size == 0:
        return {"mean": 0.0, "var": 0.0}
    return {"mean": float(np.mean(reward_arr)), "var": float(np.var(reward_arr))}
...
@register_adv_est(AdvantageEstimator.HA_DW)
def compute_ha_dw_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    batch_reward_history: Optional[list[float] | np.ndarray] = None,
    norm_adv_by_std_in_grpo: bool = True,
    epsilon: float = 1e-6,
    config: Optional[AlgoConfig] = None,
    **kwargs,
) -> tuple[torch.Tensor, torch.Tensor]:
    """HA-DW: use recent per-batch mean reward stats, then GRPO-style group normalization.

    ``batch_reward_history`` holds the last-k **batch-mean** total rewards (scalars).
    From it we compute ``hist_mean`` / ``hist_var`` and compare each prompt group's
    in-batch mean ``id2mean[g]`` to ``hist_mean`` to build a per-sample scale, then
    apply the usual GRPO centering (and optional std scaling) on raw outcome scores.
    """
    del config  # reserved; norm_adv_by_std_in_grpo passed explicitly
    ...
    with torch.no_grad():
        if batch_reward_history is None or len(batch_reward_history) == 0:
            hist_mean = torch.mean(scores)
            hist_var = torch.var(scores, unbiased=False)
        else:
            reward_stats = compute_reward_history_stats(batch_reward_history)
            hist_mean = torch.as_tensor(reward_stats["mean"], dtype=scores.dtype, device=scores.device)
            hist_var = torch.as_tensor(reward_stats["var"], dtype=scores.dtype, device=scores.device)
        ...
        for i in range(bsz):
            ...
            gstd = id2std[g]
            if norm_adv_by_std_in_grpo:
                # Group mean vs history mean (batch-level baseline from past k steps).
                hist_diff = gmean - hist_mean
                hist_scaled_direction = -torch.sign(hist_diff) * torch.sign(scores[i] - gmean)
                hist_scaled_magnitude = torch.abs(hist_diff)
                scale = 1.5 * torch.exp(hist_scaled_direction * hist_scaled_magnitude)
                scores[i] = scale * (scores[i] - gmean) / (gstd + epsilon)
            else:
