...
def compute_advantage(
    ...
    batch_reward_history: Optional[list[float]] = None,
) -> DataProto:
    ...
    """
    if batch_reward_history is not None and len(batch_reward_history) > 0:
        reward_stats = core_algos.compute_reward_history_stats(batch_reward_history)
        data.meta_info["batch_reward_hist_mean"] = reward_stats["mean"]
        data.meta_info["batch_reward_hist_var"] = reward_stats["var"]
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if batch_reward_history is not None and len(batch_reward_history) > 0:
            adv_kwargs["batch_reward_history"] = batch_reward_history
        ...
        if adv_estimator in (AdvantageEstimator.HA_DW, "ha-dw"):
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.reward_window_k = int(self.config.trainer.get("reward_window_k", self.config.trainer.get("acc_window_k", 0)))
        self.batch_reward_history = deque(maxlen=self.reward_window_k) if self.reward_window_k > 0 else None
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    batch_reward_moving_avg = None
                    batch_reward_history_for_adv = None
                    # generate a batch
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.batch_reward_history is not None:
                            # When task-level "acc" is unavailable, treat per-batch average reward as accuracy signal.
                            batch_reward_values = reward_tensor.sum(-1)
                            curr_batch_acc = float(batch_reward_values.mean().item())
                            self.batch_reward_history.append(curr_batch_acc)
                            metrics["training/batch_acc"] = curr_batch_acc
                            if len(self.batch_reward_history) > 0:
                                batch_reward_moving_avg = float(np.mean(self.batch_reward_history))
                                metrics["training/batch_reward_moving_avg"] = batch_reward_moving_avg
                                batch_reward_history_for_adv = list(self.batch_reward_history)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        batch = compute_advantage(
                            ...
                            batch_reward_history=batch_reward_history_for_adv,
                        )
